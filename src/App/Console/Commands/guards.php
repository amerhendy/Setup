...
class guards extends Command
{
    ...
    public function handle()
    {
        ...
        /////////////////////////////////////////////////////////////
        $this->line('');
        $this->info('publish Passport');
        $this->progressBar->advance();
        $this->executeArtisanProcess('migrate',[],false);
        $this->executeArtisanProcess('passport:install',[],false,$this->force);
        $this->executeArtisanProcess('passport:keys',[],false,$this->force);
        $this->line('');
        ...
        $this->executeArtisanProcess('vendor:publish',[
            '--provider'=>\Spatie\Permission\PermissionServiceProvider::class
                ...
                $this->progressBar->advance();
                $configfolder=getallfiles(config('Amer.Security.package_path').'config');
                foreach($configfolder as $a=>$b){
                    ...
                    //dd($filename,$b,$source);
                    $this->publishfiles($b,base_path('config/'.$filename.'.php'));
                }
                /////////
                $this->line('');
                ...
                foreach($migrationfolder as $a=>$b){
                    ...
                    $filename=Str::beforeLast($filename,'.');
                    $source=Str::after($b,'src');
                    $this->publishfiles($b,base_path('database/migrations/'.$filename.'.php'));
                }
    ...
    public function checkservice(){
        ...
        'laravel/passport'=>$this->get_loaded_providers('Laravel\Passport\PassportServiceProvider'),
        ];
        foreach($als as $a=>$b){
            if($b==false){$return[]=$a;}
        }
